...
module.exports.sendMassiveEmail = async(event, context, callback) => {
    var clients = [];
    var htmlBody = "";
    var tags =[];
    var subject = "";
    ...
    for (var i = 0; i < clients.length; i++) {
        ...
        promises.push(sendEmailsAsync(client, htmlBody, subject));
    }
    var response = {};
    try {
...
function processTag(htmlBody, clientName) {
    let newstr = htmlBody;
    if (newstr.includes("[[[Nombre]]]")) {
        newstr = newstr.replace("[[[Nombre]]]", clientName);
    }
    return newstr;
}
